...
if (isset($_POST["data"])) {
    ...
    if (!file_exists(__DIR__ . "/processor/" . $method . ".class.php")) {
        $log->logError("processLow > method " . $method . " not found!");
    ...
    if (class_exists($processorClass)) {
        ...
        if (method_exists($processor, $action)) {
            ...
            $processor->$action();
            $log->logInfo("processLow > $method -> action $action called");
        } else {
            $log->logError("processLow > $method -> action " . $action . " not found!");
            header('HTTP/1.0 404 Not Found');
        }
    } else {
        $log->logError("processLow > " . $method . ".class.php not found!");
        header('HTTP/1.0 404 Not Found');
    }
}
?>
