...
class HttpAuthUtils {
    ...
    public static function checkMobileHttpAuth() {
        $auth = true;
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            $auth = true;
        } else {
            if ($_SERVER['PHP_AUTH_USER'] == SettingsUtil::getSetting(SETTINGS_MOBILE_USER) && $_SERVER['PHP_AUTH_PW'] == SettingsUtil::getSetting(SETTINGS_MOBILE_USER_PASS)) {
                $auth = false;
            }
