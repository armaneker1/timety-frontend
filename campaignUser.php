...
<html dir="ltr" lang="en-US" xmlns:fb="http://www.facebook.com/2008/fbml" xmlns:og="http://opengraphprotocol.org/schema/">
    <head>
        ...
        if (!empty($user) && isset($_SESSION['findfriends']) && ($_SESSION['findfriends'] == 1 || $_SESSION['findfriends'] == '1')) {
            ...
            <script>
                jQuery(document).ready(function(){
                    //checkThis
                    openFriendsPopup(<?= $user->id ?>,<?= $puserIdS ?>,3);
                });
            </script>
        <?php } ?>
        <!-- Open find friends -->
        ...
        if (!empty($prm_event)) {
            ...
            <meta property="description" content="<?= $prm_event->description ?>"/>
        ...
        ?>');
            ...
            <meta property="description" content="<?= $p_user->about ?>"/>
    ...
    <body class="bg <?= LanguageUtils::getLocale() . "_class" ?>" itemscope="itemscope" itemtype="http://schema.org/WebPage">
        ...
        <script>
            setTimeout(function(){
                analytics_setProperty("userpage", false); 
                analytics_setProperty("userpageId", '0'); 
                analytics_setProperty("campaign", true); 
                analytics_setProperty("campaignId", '<?= $p_user_id ?>'); 
            },300);
        </script>
        <div class="main_sol">
            <?php
