...
if (isset($_POST["data"])) {
    ...
    if (class_exists($processorClass)) {
        ...
        if (method_exists($processor, $action)) {
            ...
            $processor->$action();
            $log->logInfo("processHigh > $method -> action $action called");
        } else {
            $log->logError("processHigh > $method -> action " . $action . " not found!");
            header('HTTP/1.0 404 Not Found');
        }
    } else {
        $log->logError("processHigh > " . $method . ".class.php not found!");
        header('HTTP/1.0 404 Not Found');
    }
}
?>
